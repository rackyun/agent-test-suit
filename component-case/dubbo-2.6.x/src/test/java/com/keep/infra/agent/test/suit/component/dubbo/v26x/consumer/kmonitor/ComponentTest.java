package com.keep.infra.agent.test.suit.component.dubbo.v26x.consumer.kmonitor;
...
public class ComponentTest extends AbstractKeepMonitorComponentTest {
    ...
    public void verifyComponent() throws Exception {
        ...
        FalconMetricEntry[] targetMetric = FalconMetricUtil.findMetric(metricEntries, "rpc.client.timer.count");
        assertTrue("rpc client metric is not exist", targetMetric.length > 0);
        targetMetric = FalconMetricUtil.findMetric(metricEntries, "rpc.server.timer.count");
        assertTrue("rpc server metric is not exist", targetMetric.length > 0);
    }
