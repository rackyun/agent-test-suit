package com.keep.infra.agent.test.suit.component.monitor.v22.kmonitor;
...
public class ComponentTest extends AbstractKeepMonitorComponentTest {
    ...
    private static final String TEST_API_PATH = "/test/monitor/v22/run";
    ...
    public void verifyComponent() throws Exception {
        ...
        FalconMetricEntry[] targetMetric = FalconMetricUtil.findMetric(metricEntries, "custom.monitor.request.timer.mean");
        assertTrue("custom.monitor metric is not exist", targetMetric.length > 0);
        targetMetric = FalconMetricUtil.findMetric(metricEntries, "custom.monitor.invoke.count");
        assertTrue("custom.monitor.invoke.count metric is not exist", targetMetric.length > 0);
    }
