package com.keep.infra.agent.test.suit.component.tomcat.v78.kmonitor;
...
public class ComponentTest extends AbstractKeepMonitorComponentTest {
    ...
    private static final String TEST_API_PATH = "/test/tomcat/v78/run";
    ...
    public void verifyComponent() throws Exception {
        ...
        FalconMetricEntry[] targetMetric = FalconMetricUtil.findMetric(metricEntries, "http.server.timer.count");
        assertTrue("tomcat server metric is not exist", targetMetric.length > 0);
        targetMetric = FalconMetricUtil.findMetric(metricEntries, "tomcat.connector.maxThreads");
        assertTrue("tomcat maxThreads metric is not exist", targetMetric.length > 0);
    }
